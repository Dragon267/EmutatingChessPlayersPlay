...
model = tf.keras.models.load_model(f'{name}_model.model')
...
try:
    text_board = convert_to_text(board).split()
    cur = []
    for elem in text_board:
        cur.append(int(elem))
    pred = np.argmax(model.predict([cur])[0])
    x = pred // 64
    y = pred % 64
    ...
    bot_move = f'{getSimbol(xx)}{xy}{getSimbol(yx)}{yy}'
    print(bot_move)
